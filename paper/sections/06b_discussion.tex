...
\aurocNaiveFull; \S\ref{sec:res-value}). The full pipeline's AUROC is higher, but
the planned paired comparison is not statistically resolved
($\Delta$AUROC \aurocDiffFullNaive, 95\% CI \aurocDiffCI, \aurocDiffPrel), so the
study does not prove either superiority or equivalence. Read that way, the result
points to a different bottleneck: with or without our prompt, the model already
produces an outcome-aligned first-pass score in this setting---raw judgment is not what this
...
quality signal in this setting?''---which the strong naive baseline answers yes---to
...
control for this by construction in the choice of cohort: the grading model
(\texttt{gpt-5.4}) has a knowledge cutoff of August~2025, whereas the primary
cohort's (ICLR~2026) decisions and reviews were released in January~2026, so the
outcome could not have been in training. We add a direct robustness check: we flag
submissions whose arXiv preprint predates the cutoff and report the headline
metrics with and without them, bounding any residual \emph{paper-text} leakage (the
model seeing the manuscript, as a reviewer would, is not itself a confound;
memorizing its reception is). This arXiv-before-cutoff sensitivity split is reported
in Appendix~\ref{app:robust}. Two further checks are planned but deferred to future
work (\S\ref{sec:limits}): a fully pre-cutoff ICLR~2025 cohort as a contaminated
contrast---if the score--outcome relationship is no stronger there than on the clean
2026 cohort, memorization is not driving it---and a prospective confirmation that
grades a future venue's submissions before its decisions are released, making
leakage logically impossible. The
