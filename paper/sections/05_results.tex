...
\aurocDiffPrel; $n=\naiveN$); the score distributions split by the human outcome are
at least as well separated for AIPR as for the naive judge (Fig.~\ref{fig:naive}a).
The validity is therefore not manufactured by the engineering: \emph{with or without
our prompt, a frontier model is already intelligent enough to produce a trustworthy
first-pass reviewing signal---intelligence is not the bottleneck}, and the reviewer,
not the model, keeps the decision. This also settles the pre-registered V1
...
on. The scoring rule does several things at once and pays no accuracy penalty for
it: the validated overall is a by-product of the pass that yields the review
(\opSixtyFullAcc\% vs.\ \opSixtyNaiveAcc\% balanced accuracy at the pre-registered
AIPR@60 operating point confirms the pipeline is no worse at the decision itself).
The value of AIPR is therefore the processing it wraps around an already-capable
model, and the case for building it does not rest on out-scoring that model.
...
\begin{figure}[t]
  ...
  \textbf{(a)} Overall-score distributions split by the human accept/reject
  outcome for each grader; the paired AUROC difference is annotated and not
  significant---AIPR separates the classes at least as well as the bare prompt,
  not better. \textbf{(b)} Within-paper score SD over repeated runs---AIPR grades
  far more consistently. A capable model already scores competently (a); the
  engineered pipeline's contribution is reliability (b) and a grounded,
  rubric-anchored review produced in the same pass. Each method's
...
manuscripts---an empty audit result is recorded as a perfect bibliography---which
we diagnose in \S\ref{sec:discussion} and treat as a limitation in
\S\ref{sec:limits}.
