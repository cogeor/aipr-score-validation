...
(AUROC~\aurocNaiveFull), and the full pipeline discriminates no better (paired
$\Delta$AUROC \aurocDiffFullNaive, 95\% CI \aurocDiffCI, \aurocDiffPrel).
\emph{With or without our prompt, a frontier model is already intelligent enough to
produce a trustworthy first-pass reviewing signal}: \emph{intelligence is not the
bottleneck}, and the human keeps the decision. What the engineered pipeline adds is
